                               ...
                               fvv_bitstream_t             *data)
{
  *self           = (fvv_attribute_information_t){0};
  ...
  FVV_SET_SETTER_PTR(fvv_attribute_information_t,
                     ai_attribute_count,
  ...
  for (i = 0; i < self->ai_attribute_count[atlasID]; i++)
  {
    ...
    if (self->vps->vps_auxiliary_video_present_flag[atlasID])
    {
      buff->write_bits(
          ...
          FVV_DESCRIPTOR_AI_AUXILIARY_ATTRIBUTE_CODEC_ID);
    }
    if (self->vps->vps_map_count_minus1[atlasID] > 0)
    {
      buff->write_bits(
          ...
          FVV_DESCRIPTOR_AI_ATTRIBUTE_MAP_ABSOLUTE_CODING_PERSISTENCE_FLAG);
    }
    buff->write_bits(buff,
                     self->ai_attribute_dimension_minus1[atlasID][i],
                     FVV_BIT_AI_ATTRIBUTE_DIMENSION_MINUS1,
                     FVV_DESCRIPTOR_AI_ATTRIBUTE_DIMENSION_MINUS1);
    if (self->ai_attribute_dimension_minus1[atlasID][i] > 0)
    {
      buff->write_bits(
          ...
          FVV_DESCRIPTOR_AI_ATTRIBUTE_DIMENSION_PARTITIONS_MINUS1);
      ...
      for (j = 0; j < k; j++)
      {
        ...
        else
        {
          buff->write_bits(
              ...
              FVV_DESCRIPTOR_AI_ATTRIBUTE_PARTITION_CHANNELS_MINUS1);
        }
        remainingDimensions -=
            self->ai_attribute_partition_channels_minus1[atlasID][i]
                                                        [j] +
    ...
    buff->write_bits(
        ...
        FVV_DESCRIPTOR_AI_ATTRIBUTE_2D_BIT_DEPTH_MINUS1);
    buff->write_bits(buff,
                     self->ai_attribute_MSB_align_flag[atlasID][i],
                     FVV_BIT_AI_ATTRIBUTE_MSB_ALIGN_FLAG,
                     FVV_DESCRIPTOR_AI_ATTRIBUTE_MSB_ALIGN_FLAG);
  }
  return FVV_RET_SUCCESS;
}
    ...
    fvv_attribute_information_t *other)
{
  ...
  self->set_ai_attribute_codec_id(self, other->ai_attribute_codec_id);
  ...
  self->set_ai_attribute_map_absolute_coding_persistence_flag(
      self, other->ai_attribute_map_absolute_coding_persistence_flag);
...
FVV_DEFINE_1D_STATIC_ARR_SETTER(fvv_attribute_information_t,
                                ai_attribute_count,
                                0x1 << FVV_BIT_VPS_ATLAS_ID);
FVV_DEFINE_2D_STATIC_ARR_SETTER(fvv_attribute_information_t,
                                ai_attribute_type_id,
                                0x1 << FVV_BIT_VPS_ATLAS_ID,
                                0x1 << FVV_BIT_AI_ATTRIBUTE_COUNT);
FVV_DEFINE_2D_STATIC_ARR_SETTER(fvv_attribute_information_t,
...
FVV_DEFINE_2D_STATIC_ARR_SETTER(fvv_attribute_information_t,
                                ...
                                0x1 << FVV_BIT_VPS_ATLAS_ID,
                                0x1 << FVV_BIT_AI_ATTRIBUTE_COUNT);
FVV_DEFINE_3D_STATIC_ARR_SETTER(
    fvv_attribute_information_t,
    ai_attribute_partition_channels_minus1,
    0x1 << FVV_BIT_VPS_ATLAS_ID,
    0x1 << FVV_BIT_AI_ATTRIBUTE_COUNT,
