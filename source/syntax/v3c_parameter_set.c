                                     ...
                                     fvv_bitstream_t         *data)
{
  *self           = (fvv_v3c_parameter_set_t){0};
  ...
  FVV_SET_SETTER_PTR(fvv_v3c_parameter_set_t,
                     vps_v3c_parameter_set_id);
  FVV_SET_SETTER_PTR(fvv_v3c_parameter_set_t,
                     vps_reserved_zero_8bits);
  ...
  FVV_SET_SETTER_PTR(fvv_v3c_parameter_set_t,
                     ...
                     0x1 << FVV_BIT_VPS_MAP_COUNT_MINUS1);
  FVV_SET_SETTER_PTR(
      fvv_v3c_parameter_set_t, ptl, fvv_profile_tier_level_t);
  FVV_SET_SETTER_PTR(
      fvv_v3c_parameter_set_t, oi, fvv_occupancy_information_t);
  FVV_SET_SETTER_PTR(
      fvv_v3c_parameter_set_t, gi, fvv_geometry_information_t);
  FVV_SET_SETTER_PTR(
      fvv_v3c_parameter_set_t, ai, fvv_attribute_information_t);
  FVV_SET_SETTER_PTR(
      fvv_v3c_parameter_set_t, ba, fvv_byte_alignment_t);
...
fvv_ret_t fvv_v3c_parameter_set_destroy(fvv_v3c_parameter_set_t *self)
{
  if (!self)
  {
...
fvv_ret_t fvv_v3c_parameter_set_pack(fvv_v3c_parameter_set_t *self)
{
  ...
  buff->write_bits(buff,
                   ...
                   FVV_DESCRIPTOR_VPS_V3C_PARAMETER_SET_ID);
  buff->write_bits(buff,
                   self->vps_reserved_zero_8bits,
                   FVV_BIT_VPS_RESERVED_ZERO_8BITS,
                   FVV_DESCRIPTOR_VPS_RESERVED_ZERO_8BITS);
  buff->write_bits(buff,
                   self->vps_atlas_count_minus1,
                   FVV_BIT_VPS_ATLAS_COUNT_MINUS1,
                   FVV_DESCRIPTOR_VPS_ATLAS_COUNT_MINUS1);
  for (k = 0; k < self->vps_atlas_count_minus1 + 1; k++)
  {
    buff->write_bits(buff,
                     self->vps_atlas_id[k],
                     FVV_BIT_VPS_ATLAS_ID,
                     FVV_DESCRIPTOR_VPS_ATLAS_ID);
    j = self->vps_atlas_id[k];
    buff->write_bits(buff,
                     self->vps_frame_width[j],
                     FVV_BIT_VPS_FRAME_WIDTH,
                     FVV_DESCRIPTOR_VPS_FRAME_WIDTH);
    buff->write_bits(buff,
                     self->vps_frame_height[j],
                     FVV_BIT_VPS_FRAME_HEIGHT,
                     FVV_DESCRIPTOR_VPS_FRAME_HEIGHT);
    buff->write_bits(buff,
                     self->vps_map_count_minus1[j],
                     FVV_BIT_VPS_MAP_COUNT_MINUS1,
                     FVV_DESCRIPTOR_VPS_MAP_COUNT_MINUS1);
    if (self->vps_map_count_minus1[j] > 0)
    {
      buff->write_bits(
          ...
          FVV_DESCRIPTOR_VPS_MULTIPLE_MAP_STREAMS_PRESENT_FLAG);
    }
    ...
    for (i = 1; i <= self->vps_map_count_minus1[j]; i++)
    {
      if (self->vps_multiple_map_streams_present_flag[j])
      {
        buff->write_bits(
            ...
            FVV_DESCRIPTOR_VPS_MAP_ABSOLUTE_CODING_ENABLED_FLAG);
      }
      else
      {
                                ...
                                fvv_v3c_parameter_set_t *other)
{
  ...
  self->set_vps_v3c_parameter_set_id(self,
                                     other->vps_v3c_parameter_set_id);
  self->set_vps_reserved_zero_8bits(self,
                                    other->vps_reserved_zero_8bits);
  self->set_vps_atlas_count_minus1(self,
                                   other->vps_atlas_count_minus1);
  self->set_vps_extension_present_flag(
      self, other->vps_extension_present_flag);
  self->set_vps_extension_8bits(self, other->vps_extension_8bits);
  self->set_vps_extension_length_minus1(
      self, other->vps_extension_length_minus1);
  self->set_vps_extension_data_byte(self,
                                    other->vps_extension_data_byte);
  ...
  self->set_vps_multiple_map_streams_present_flag(
      self, other->vps_multiple_map_streams_present_flag);
  self->set_vps_auxiliary_video_present_flag(
      self, other->vps_auxiliary_video_present_flag);
  self->set_vps_occupancy_video_present_flag(
      self, other->vps_occupancy_video_present_flag);
  self->set_vps_geometry_video_present_flag(
      self, other->vps_geometry_video_present_flag);
  self->set_vps_attribute_video_present_flag(
      self, other->vps_attribute_video_present_flag);
  self->set_vps_map_absolute_coding_enabled_flag(
      self, other->vps_map_absolute_coding_enabled_flag);
  self->set_vps_map_predictor_index_diff(
      self, other->vps_map_predictor_index_diff);
...
FVV_DEFINE_SCALAR_SETTER(fvv_v3c_parameter_set_t,
                         vps_v3c_parameter_set_id);
FVV_DEFINE_SCALAR_SETTER(fvv_v3c_parameter_set_t,
                         vps_reserved_zero_8bits);
FVV_DEFINE_SCALAR_SETTER(fvv_v3c_parameter_set_t,
...
FVV_DEFINE_SCALAR_SETTER(fvv_v3c_parameter_set_t,
                         vps_extension_data_byte);
FVV_DEFINE_1D_STATIC_ARR_SETTER(fvv_v3c_parameter_set_t,
                                vps_atlas_id,
                                (0x1
                                 << FVV_BIT_VPS_ATLAS_COUNT_MINUS1) +
                                    1);
...
FVV_DEFINE_1D_STATIC_ARR_SETTER(fvv_v3c_parameter_set_t,
                                vps_attribute_video_present_flag,
                                0x1 << FVV_BIT_VPS_ATLAS_ID);
FVV_DEFINE_2D_STATIC_ARR_SETTER(fvv_v3c_parameter_set_t,
                                vps_map_absolute_coding_enabled_flag,
                                0x1 << FVV_BIT_VPS_ATLAS_ID,
                                0x1 << FVV_BIT_VPS_MAP_COUNT_MINUS1);
FVV_DEFINE_2D_STATIC_ARR_SETTER(fvv_v3c_parameter_set_t,
                                ...
                                0x1 << FVV_BIT_VPS_ATLAS_ID,
                                0x1 << FVV_BIT_VPS_MAP_COUNT_MINUS1);
FVV_DEFINE_OBJ_SETTER(fvv_v3c_parameter_set_t,
                      ptl,
                      fvv_profile_tier_level_t);
FVV_DEFINE_OBJ_SETTER(fvv_v3c_parameter_set_t,
                      oi,
