...
fvv_ret_t fvv_nal_unit_header_pack(fvv_nal_unit_header_t *self)
{
  ...
  buff->write_bits(buff,
                   ...
                   FVV_DESCRIPTOR_NAL_FORBIDDEN_ZERO_BIT);
  buff->write_bits(buff,
                   self->nal_unit_type,
                   FVV_BIT_NAL_UNIT_TYPE,
                   FVV_DESCRIPTOR_NAL_UNIT_TYPE);
  buff->write_bits(buff,
                   self->nal_layer_id,
                   FVV_BIT_NAL_LAYER_ID,
                   FVV_DESCRIPTOR_NAL_LAYER_ID);
  buff->write_bits(buff,
                   self->nal_temporal_id_plus1,
                   FVV_BIT_NAL_TEMPORAL_ID_PLUS1,
                   FVV_DESCRIPTOR_NAL_TEMPORAL_ID_PLUS1);
  return FVV_RET_SUCCESS;
}
                                        ...
                                        fvv_nal_unit_header_t *other)
{
  ...
  self->set_nal_temporal_id_plus1(self, other->nal_temporal_id_plus1);
  return FVV_RET_SUCCESS;
}
FVV_DEFINE_SCALAR_SETTER(fvv_nal_unit_header_t,
