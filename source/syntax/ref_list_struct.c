                                   ...
                                   uint64_t               rlsIdx)
{
  ...
  uint64_t         i    = 0;
  buff                  = self->data;
  buff->write_bits(buff, self->num_ref_entries[rlsIdx], FVV_BIT_NUM_REF_ENTRIES);
  for (i = 0; i < num_ref_entries[rlsIdx]; i++)
  {
    if (self->aspsr->asps_long_term_ref_atlas_frames_flag)
      buff->write_bits(buff, self->st_ref_atlas_frame_flag[rlsIdx][i], FVV_BIT_ST_REF_ATLAS_FRAME_FLAG);
    if (self->st_ref_atlas_frame_flag[rlsIdx][i])
    {
      buff->write_bits(buff, self->abs_delta_afoc_st[rlsIdx][i], FVV_BIT_ABS_DELTA_AFOC_ST);
      if (self->abs_delta_afoc_st[rlsIdx][i] > 0)
        buff->write_bits(buff, self->straf_entry_sign_flag[rlsIdx][i], FVV_BIT_STRAF_ENTRY_SIGN_FLAG);
    }
    else
      buff->write_bits(buff, self->afoc_lsb_lt[rlsIdx][i], FVV_BIT_AFOC_LSB_LT);
  }
  return FVV_RET_SUCCESS;
}
fvv_ret_t fvv_ref_list_struct_copy_from(fvv_ref_list_struct_t *self, fvv_ref_list_struct_t *other)
{
  ...
  self->set_num_ref_entries(self, other->num_ref_entries, other->num_ref_entries_size);
  self->set_st_ref_atlas_frame_flag(self, other->st_ref_atlas_frame_flag, other->st_ref_atlas_frame_flag_size);
  self->set_abs_delta_afoc_st(self, other->abs_delta_afoc_st, other->abs_delta_afoc_st_size);
  self->set_straf_entry_sign_flag(self, other->straf_entry_sign_flag, other->straf_entry_sign_flag_size);
  self->set_afoc_lsb_lt(self, other->afoc_lsb_lt, other->afoc_lsb_lt_size);
  return FVV_RET_SUCCESS;
}
